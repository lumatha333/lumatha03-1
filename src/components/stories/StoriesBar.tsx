...
const MobileFilterChip = memo(({ id, label, color }: { id: string; label: string; color: string }) => {
  return (
    <button
      className={`flex items-center gap-1 shrink-0 px-3 py-1.5 rounded-full border border-white/10 hover:border-white/20 transition-all duration-300 active:scale-90 group bg-gradient-to-br ${color}`}
    >
...
const DesktopCategoryButton = memo(({ icon: Icon, label, color, textColor }: { icon: any; label: string; color: string; textColor: string }) => {
  return (
    <button
      className={`flex flex-col items-center gap-1 shrink-0 px-3 py-2 rounded-lg ${color} hover:opacity-80 transition-all duration-300 active:scale-90 group`}
      title={label}
    >
