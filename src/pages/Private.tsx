...
export default function Private() {
  ...
  const filteredPosts = useMemo(() => {
    let filtered = posts.filter((p) => p.visibility === 'private' && p.user_id === user?.id);
    ...
    if (sortBy === 'most-liked') {
      ...
      filtered = [...filtered].sort((a, b) => {
        const aSaved = saved.includes(a.id) ? 1 : 0;
        const bSaved = saved.includes(b.id) ? 1 : 0;
        return bSaved - aSaved;
      });
    }

    return filtered;
  }, [posts, search, user?.id, sortBy, likesCount, saved]);
