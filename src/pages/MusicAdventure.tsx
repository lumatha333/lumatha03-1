...
import {
 ...
 ArrowLeft, Camera, Send, X, Image as ImageIcon, UserCircle, Video, Mic
} from 'lucide-react';
    ...
    <div className="flex flex-col h-[70vh] bg-slate-950/50 rounded-[32px] border border-white/5 overflow-hidden shadow-2xl relative">
      ...
      <div className="p-4 border-b border-white/5 flex items-center justify-between bg-slate-900/40 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" className="h-9 w-9 rounded-xl bg-white/5 text-slate-400 hover:text-primary transition-all p-0"><Video className="w-5 h-5" /></Button>
          <Button variant="ghost" size="sm" className="h-9 w-9 rounded-xl bg-white/5 text-slate-400 hover:text-primary transition-all p-0"><Mic className="w-5 h-5" /></Button>
          <Button variant="ghost" size="sm" className="h-9 w-9 rounded-xl bg-white/5 text-slate-400 hover:text-primary transition-all p-0" onClick={captureMoment}><Camera className="w-5 h-5" /></Button>
          <Button variant="ghost" size="sm" className="h-9 w-9 rounded-xl bg-white/5 text-slate-400 hover:text-primary transition-all p-0"><MoreVertical className="w-5 h-5" /></Button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-slate-200">
      ...
      <motion.div 
        ...
        className="fixed top-0 left-0 right-0 z-[100] w-full bg-[#0a0f1e]/90 backdrop-blur-2xl border-b border-white/5 shadow-2xl shadow-black/60 h-[72px]"
      >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {places.slice(0, 48).map((place) => (
                  <motion.div 
                    ...
                    className="group relative aspect-[4/5] overflow-hidden bg-slate-900 rounded-[24px] border border-white/5 cursor-pointer shadow-xl transition-all hover:scale-[1.02] active:scale-95"
                  >
                    <img src={place.image || FALLBACK_PLACE_IMAGE} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt={place.name} />
